...
#if defined(NDEBUG)
# error "Coin3Fly cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc49aa86376f62bedb43f0c35fe4c51a66a7badbaa2c8e60cdef327496a543b1e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Coin3Fly: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Coin3Fly Signed Message:\n";
...
{
    // Coin3Fly: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Coin3Fly: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Coin3Fly: 3 days
static const int64 nTargetSpacing = 2 * 60; // Coin3Fly: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Coin3Fly: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Coin3Fly: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x06575afd5e7081e3e11be83fa5f584894cae36295afeffb5fde88ed95a73cae1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "16/March/2021 Pandemic era ends";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("042e99b39d876bade88ea2799de52cbd5674aa0425c321a17f09de68639b8928ded63da33257bbf9d5151a684da0ab01b5f5aa1cba2c1ede74ae0d52242a514e1a") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1615932328;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084587157;

        if (fTestNet)
        {
            block.nTime    = 1615932328;
            block.nNonce   = 386418195;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x12abf19d6ce3f3206e8756f7f2f61d4c146c61f6c20b5fbdd1d40f40d6326c9e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfc, 0xc5, 0xb3, 0xda }; // Coin3Fly: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("Coin3FlyMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("Coin3FlyMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static Coin3FlyMiner(CWallet *pwallet)
{
    printf("Coin3FlyMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Coin3Fly-miner");
    ...
    try { loop {
        ...
        printf("Running Coin3FlyMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("Coin3FlyMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&Coin3FlyMiner, pwallet));
}
